# P03.CP01 — CORRECTED RENDERING (append-only, see DEF-020)

The sealed `CP01-checkpoint.md` lost three backticked terms to unquoted-heredoc
command substitution. It is NOT edited (its hash is recorded evidence). This
document restores the damaged lines verbatim; every other line is identical.

Restored:
- NEG04 declared reason `LEDGER_TAMPERED`; NEG05 declared reason `RENDER_DRIFT`
- Additive `migrate` added `gate_results` and `result_schema_version`
...
no evidence, missing path, empty file, and self-referential artifact. NEG04
 on live bytes with reason `LEDGER_TAMPERED`. NEG05 `RENDER_DRIFT` on hand-edited markdown.
