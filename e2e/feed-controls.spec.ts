  ...
  await page.route("**/api/trpc/**", async route => {
    ...
    const body = operations.map((operation, index) => {
      ...
      if (operation === "games.list")
        json = input[index]?.json?.gameDate !== DATE ? [] : input[index]?.json?.sport === "NCAAF" ? games : [{ ...games[0], id: 7000001, sport: "MLB", awayTeam: "NYY", homeTeam: "BOS", gameStatus: "live" }];
    ...
    if (operations.includes("games.list") && Object.values(input).some((value: any) => value.json?.gameDate !== DATE)) await new Promise(resolve => setTimeout(resolve, 700));
...
const cards = (page: Page) => page.locator("#dmf-league-NCAAF .projection-card");
async function open(page: Page) {
  await stub(page);
  await page.goto(route);
  await expect(cards(page)).toHaveCount(68);
  await expect(page.getByRole("region", { name: "Feed controls" })).toBeVisible();
}
async function overflow(page: Page) {
  expect(await page.evaluate(() => document.documentElement.scrollWidth - innerWidth)).toBeLessThanOrEqual(1);
  expect(await page.locator(".feed-toolbar").evaluateAll(elements => elements.filter(element => element.scrollWidth > element.clientWidth + 1).map(element => element.className))).toEqual([]);
}

test("composes URL filters, restores back navigation and clears old games on date change", async ({ page }) => {
  await open(page);
  await expect(page.locator(".dmf-league").first()).toHaveAttribute("id", "dmf-league-NCAAF");
  await page.getByLabel("Status", { exact: true }).selectOption("final");
  await expect(cards(page)).toHaveCount(games.filter(game => game.gameStatus === "final").length);
  await expect(page).toHaveURL(/status=final/);
  await page.getByLabel("Status", { exact: true }).selectOption("all");
  await page.getByLabel("Sport / league", { exact: true }).selectOption("NCAAF");
  await page.getByLabel("Conference", { exact: true }).selectOption({ label: "Big Ten Conference" });
  ...
  await expect(page.getByText("No games for this date", { exact: true })).toBeVisible();
  await page.goBack();
  await expect(cards(page)).toHaveCount(1);
  await expect(page.getByRole("button", { name: "Game Ball State at Ohio State" })).toBeVisible();
  ...
  await expect(page.getByLabel("Conference", { exact: true })).toHaveValue("all");
  await expect(page).not.toHaveURL(/game=/);
});

test("calendar keyboard focus and expanded pricing preserve grid geometry", async ({ page }) => {
  await open(page);
  const date = page.getByRole("button", { name: /^Choose date/ });
  await date.click();
  await expect(page.getByRole("dialog", { name: "Choose feed date" })).toBeVisible();
  ...
  await expect(first.locator(".market-table, .market-table__basis")).toHaveCount(0);
  const before = await first.boundingBox();
  const trigger = first.getByRole("button", { name: "View full AI model projections" });
  await trigger.click();
  const dialog = page.getByRole("dialog", { name: "Bryant at Army model projections" });
  await expect(dialog).toBeVisible();
  await expect(dialog.locator(".market-table__basis").first()).toHaveText("at +37");
  expect((await first.boundingBox())?.height).toBe(before?.height);
  await page.screenshot({ path: `${output}/projections.png` });
  await dialog.getByRole("button", { name: "Close full model projections" }).click();
  await expect(trigger).toBeFocused();
});

for (const [width, height] of [[320, 812], [375, 812], [768, 1024], [1024, 768], [1272, 827], [1440, 900], [1920, 1080]]) {
  test(`compact ordered cards and adaptive names at ${width}x${height}`, async ({ page }) => {
    await page.setViewportSize({ width, height });
    await open(page);
    await page.evaluate(() => document.fonts.ready);
    await overflow(page);
    const geometry = await cards(page).evaluateAll(elements => elements.slice(0, 6).map(element => {
      const box = element.getBoundingClientRect();
      return { top: box.top, bottom: box.bottom, height: box.height };
    }));
    for (const item of geometry) {
      expect(item.height).toBeLessThan(490);
      const sameRow = geometry.filter(other => Math.abs(other.top - item.top) < 1);
      expect(Math.max(...sameRow.map(other => other.bottom)) - Math.min(...sameRow.map(other => other.bottom))).toBeLessThanOrEqual(1);
    }
    const fits = await page.locator(".matchup__line:not(.matchup__line--measure)").evaluateAll(elements => elements.map(element => {
      const center = element.closest(".matchup__center")!;
      const measurement = center.querySelector(".matchup__line--measure")!;
      return { compact: element.getAttribute("data-compact"), fullFits: measurement.getBoundingClientRect().width <= center.clientWidth, width: element.getBoundingClientRect().width, available: center.clientWidth };
    }));
    ...
    await expect(cards(page)).toHaveCount(68);
    await overflow(page);
    expect(await cards(page).first().locator(".projection-card__live-dot").count()).toBe(0);
    if (width === 375) await page.screenshot({ path: `${output}/feed-light-${width}.png` });
  });
}
