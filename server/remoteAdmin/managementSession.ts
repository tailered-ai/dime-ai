...
export const MANAGEMENT_METHODS: Record<string, string> = {
  "appUsers.me": "GET", "appUsers.listUsers": "GET", "subscriptionPlans.list": "GET",
  "appUsers.createUser": "POST", "appUsers.updateUser": "POST", "appUsers.deleteUser": "POST",
  "appUsers.forceLogoutUser": "POST", "appUsers.forceLogoutAll": "POST",
  "appUsers.adminDisconnectDiscord": "POST", "appUsers.generateDiscordInvite": "POST",
  "appUsers.syncDiscordRole": "POST", "appUsers.setManualDiscordId": "POST",
};
...
export async function getManagementSession(req: Request) {
  ...
  const deny = () => new TRPCError({ code: "UNAUTHORIZED", message: "Invalid Tailered management proof" });
  ...
  const now = Date.now();
  if (!secret || actor !== MANAGEMENT_ACTOR || !Number.isSafeInteger(sentAt) || Math.abs(now - sentAt) > WINDOW_MS ||
      !/^[0-9a-f-]{36}$/.test(nonce) || MANAGEMENT_METHODS[procedure] !== req.method ||
      url.pathname !== `/api/trpc/${procedure}` || Array.from(url.searchParams.keys()).some(key => key !== "input")) throw deny();
  const input = req.method === "GET" ? url.searchParams.get("input") : (req.body ?? null);
  const proof = JSON.stringify({ kind: MANAGEMENT_KIND, actor, sentAt, nonce, method: req.method, path: url.pathname, input });
  if (!verifyTaileredSignature(proof, signature, secret)) throw deny();
  for (const [key, expires] of Array.from(usedNonces)) if (expires <= now) usedNonces.delete(key);
  if (usedNonces.has(nonce) || usedNonces.size >= 10_000) throw deny();
  usedNonces.set(nonce, sentAt + WINDOW_MS);

  const { getAppUserByUsername, lookupAppUserByIdFresh } = await import("../db");
  ...
  if (!user || user.username !== "prez" || user.discordId !== MANAGEMENT_ACTOR.split("@")[0] ||
      user.role !== "owner" || !user.hasAccess || user.deletedAt ||
      (user.expiryDate !== null && user.expiryDate <= now)) throw deny();
  return { userId: user.id, role: user.role, tv: user.tokenVersion, exp: sentAt + WINDOW_MS };
}
